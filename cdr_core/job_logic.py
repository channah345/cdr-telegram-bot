def get_assigned_engineer_ids(fields: dict) -> list[str]:
    ...
    def add(value):
        ...
        if value and value.lower() not in ["none", "null"] and value not in seen:
            assigned.append(value); seen.add(value)
    for engineer in fields.get("Engineer", []) if isinstance(fields.get("Engineer", []), list) else []:
...
def is_closed_job(fields: dict) -> bool:
    ...
    open_statuses = {"", AWAITING_DEPLOYMENT_STATUS, LEGACY_AWAITING_DEPLOYMENT_STATUS, ASSIGNED_STATUS, TRAVELLING_STATUS, ON_SITE_STATUS}
...
def engineer_has_logged(fields: dict, engineer_name: str, action: str) -> bool:
    ...
    return f" - {engineer_name} - {action}" in log
